...
@section('page-title', 'Manajemen Anggota Kelas')
...
<div class="container-fluid px-0">
        ...
        <div class="card-body p-4 d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
            <div class="d-flex align-items-center">
                <div class="icon-box rounded-4 me-3 d-flex align-items-center justify-content-center" 
                    ...
                    <i class="bi bi-person-plus fs-4" style="color: #134B46;"></i>
                </div>
                <div>
                    <h5 class="fw-bold mb-0">Penempatan Kelas</h5>
                    <p class="text-secondary small mb-0">Atur pembagian siswa ke dalam ruang kelas masing-masing</p>
                </div>
            </div>
    ...
    <div class="card border-0 shadow-sm rounded-card overflow-hidden">
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0 text-center" id="memberTable">
                <thead class="bg-light">
                    <tr>
                        <th class="ps-4 py-3 text-secondary small fw-bold text-uppercase">No</th>
                        <th class="py-3 text-secondary small fw-bold text-uppercase text-start">Siswa</th>
                        <th class="py-3 text-secondary small fw-bold text-uppercase">Kelas</th>
                ...
                <tbody>
                    ...
                    <tr class="member-row">
                        ...
                        <td class="text-start">
                            <div class="d-flex align-items-center">
                                @php $style = $avatarStyles[$row->id % count($avatarStyles)]; @endphp
                                ...
                                <div>
                                    <div class="fw-bold text-dark search-name">{{ $row->siswa->nama_siswa }}</div>
                                    <div class="text-secondary small">NISN: {{ $row->siswa->nisn }}</div>
                                </div>
                            </div>
                        </td>
                        <td>
                            <span class="badge bg-light text-dark border rounded-pill px-3 search-class">{{ $row->kelas->nama_kelas }}</span>
                        </td>
                        <td class="text-center">
                            <div class="d-flex justify-content-center gap-2">
                                ...
                                <form action="{{ route('class-members.destroy', $row->id) }}" method="POST" class="d-inline">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn btn-light btn-sm rounded-3 border px-2 text-danger hover-shadow" onclick="return confirm('Keluarkan siswa ini dari kelas?')">
                    ...
                    @empty
                    <tr><td colspan="4" class="text-center py-5 text-secondary italic">Data anggota kelas kosong.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 rounded-4 shadow">
            <div class="modal-header border-0 p-4 pb-0">
                <h5 class="fw-bold text-teal-dark">Tambah Anggota Kelas</h5>
                <button type="button" class="btn-close shadow-none" data-bs-dismiss="modal"></button>
            </div>
            <form action="{{ route('class-members.store') }}" method="POST">
                @csrf
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-secondary">PILIH SISWA</label>
                        <select name="siswa_id" class="form-select rounded-3 border-0 bg-light p-3 shadow-none" required>
                            <option value="" disabled selected>-- Pilih Siswa --</option>
                            @foreach($studentsAvailable as $s)
                                <option value="{{ $s->id }}">{{ $s->nama_siswa }} ({{ $s->nisn }})</option>
                            @endforeach
                        </select>
                        <small class="text-muted mt-1 d-block" style="font-size: 10px;">*Hanya menampilkan siswa yang belum memiliki kelas.</small>
                    </div>
                    <div class="mb-3">
...
<div class="modal fade" id="modalEdit" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 rounded-4 shadow">
            <div class="modal-header border-0 p-4 pb-0">
                <h5 class="fw-bold text-teal-dark">Edit Penempatan Kelas</h5>
                <button type="button" class="btn-close shadow-none" data-bs-dismiss="modal"></button>
            </div>
            <form id="formEdit" method="POST">
                ...
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-secondary">SISWA (TETAP)</label>
                        <select name="siswa_id" id="edit_siswa" class="form-select rounded-3 border-0 bg-light p-3 shadow-none" style="pointer-events: none; opacity: 0.7;">
                            @foreach($data as $row)
                                <option value="{{ $row->siswa->id }}">{{ $row->siswa->nama_siswa }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-secondary">PINDAH KE KELAS</label>
                        <select name="kelas_id" id="edit_kelas" class="form-select rounded-3 border-0 bg-light p-3 shadow-none" required>
                            @foreach($classes as $k)
                                <option value="{{ $k->id }}">{{ $k->nama_kelas }}</option>
                            @endforeach
                        </select>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if(searchInput) {
            searchInput.addEventListener('keyup', function() {
                ...
                rows.forEach(row => {
                    ...
                    const kls = row.querySelector('.search-class').textContent.toLowerCase();
                    row.style.display = (name.includes(query) || kls.includes(query)) ? '' : 'none';
                });
            });
        }
        ...
        editButtons.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                document.getElementById('formEdit').action = `/class-members/${id}`;
                document.getElementById('edit_siswa').value = this.getAttribute('data-siswa');
